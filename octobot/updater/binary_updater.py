...
import kenbot.updater.updater as updater_class
import kenbot_commons.aiohttp_util as aiohttp_util
import kenbot_commons.constants as commons_constants
import kenbot_commons.enums as commons_enums
...
class BinaryUpdater(updater_class.Updater):
    ...
    def _get_latest_release_url(self):
        return f"{commons_constants.GITHUB_API_CONTENT_URL}/repos/" \
               ...
               f"{constants.kenbot_BINARY_PROJECT_NAME}/releases/latest"
    ...
    async def _download_binary(self):
        ...
        if matching_asset_binary is None:
            ...
            return None
        self.logger.info(f"Start downloading kenbot update at {new_binary_file_url}")
        ...
        self.logger.info(f"kenbot update downloaded successfully")
    ...
    def _give_execution_rights(self, new_binary_file):
        if os_util.get_os() is not commons_enums.PlatformsName.WINDOWS:
            self.logger.info(f"Adding execution rights to updated kenbot binary")
