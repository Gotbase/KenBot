...
import kenbot_evaluators.enums as evaluator_enums

import kenbot_services.constants as service_constants

import kenbot_trading.api as trading_api

import kenbot.community.community_fields as community_fields
...
class CommunityManager:
    ...
    def __init__(self, kenbot_api):
        self.kenbot_api = kenbot_api
        self.edited_config: configuration.Configuration = kenbot_api.get_edited_config(dict_only=False)
        ...
        self.session = kenbot_api.get_aiohttp_session()
    ...
    def _init_community_config(self):
        ...
        self.exchange_managers = trading_api.get_exchange_managers_from_exchange_ids(
            self.kenbot_api.get_exchange_manager_ids())
    ...
    @staticmethod
    def background_get_id_and_register_bot(kenbot_api):
        community_manager = CommunityManager(kenbot_api)
    ...
    async def _update_session(self, retry_on_error=True):
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
            community_fields.CommunityFields.UP_TIME.value] = int(time.time() - self.kenbot_api.get_start_time())
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
    ...
    def _get_bot_community(self):
        return {
            ...
            community_fields.CommunityFields.CURRENT_SESSION.value: {
                community_fields.CommunityFields.STARTED_AT.value: int(self.kenbot_api.get_start_time()),
                community_fields.CommunityFields.UP_TIME.value: int(time.time() - self.kenbot_api.get_start_time()),
                ...
                community_fields.CommunityFields.TYPE.value: os_util.get_kenbot_type(),
    ...
    def _get_traded_volumes(self):
        ...
        if self.has_real_trader:
            trades = []
            for exchange_manager in self.exchange_managers:
                trades += trading_api.get_trade_history(exchange_manager, since=self.kenbot_api.get_start_time())
            for trade in trades:
    ...
    def _get_eval_config(self):
        tentacle_setup_config = self.kenbot_api.get_tentacles_setup_config()
        # trading mode
        config_eval = []
        if (trading_mode := self.kenbot_api.get_trading_mode()) is not None:
