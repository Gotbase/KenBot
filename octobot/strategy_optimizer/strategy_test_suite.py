...
import kenbot.strategy_optimizer as kenbot_strategy_optimizer
import kenbot.backtesting as kenbot_backtesting
import kenbot_commons.constants as commons_constants

import kenbot_backtesting.errors as backtesting_errors
...
class StrategyTestSuite(kenbot_backtesting.AbstractBacktestingTest):
    ...
    def get_test_suite_result(self):
        return kenbot_strategy_optimizer.TestSuiteResult(self._profitability_results,
                                                          self._trades_counts,
    ...
    def _handle_results(self, independent_backtesting, profitability):
        ...
        if independent_backtesting is not None:
            exchange_manager_ids = kenbot_backtesting_api.get_independent_backtesting_exchange_manager_ids(
    ...
    async def _run_backtesting_with_current_config(self, data_file_to_use):
        ...
        try:
            ...
            independent_backtesting = kenbot_backtesting_api.create_independent_backtesting(config_to_use,
                                                                                 self.tentacles_setup_config,
                                                                                 [data_file_to_use],
                                                                                 "")
            await kenbot_backtesting_api.initialize_and_run_independent_backtesting(independent_backtesting, log_errors=False)
            await kenbot_backtesting_api.join_independent_backtesting(independent_backtesting)
