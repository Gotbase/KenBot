...
import kenbot_commons.symbol_util as symbol_util
import kenbot_commons.time_frame_manager as time_frame_manager

import kenbot.backtesting as backtesting
import kenbot_backtesting.api as backtesting_api
import kenbot_backtesting.constants as backtesting_constants
import kenbot_backtesting.enums as backtesting_enums
import kenbot_backtesting.data as backtesting_data

import kenbot_evaluators.constants as evaluator_constants
...
class IndependentBacktesting:
    def __init__(self, config,
        ...
        self.kenbot_origin_config = config
        ...
        self.kenbot_backtesting = backtesting.kenbotBacktesting(self.backtesting_config,
    ...
    async def initialize_and_run(self, log_errors=True):
        try:
            ...
            await self.kenbot_backtesting.initialize_and_run()
    ...
    async def join_backtesting_updater(self, timeout=None):
        if self.kenbot_backtesting.backtesting is not None:
            await asyncio.wait_for(self.kenbot_backtesting.backtesting.time_updater.finished_event.wait(), timeout)

    async def stop(self, memory_check=False, should_raise=False):
        try:
            if not self.stopped:
                await self.kenbot_backtesting.stop(memory_check=memory_check, should_raise=should_raise)
        finally:
            self.stopped = True

    def is_in_progress(self):
        if self.kenbot_backtesting.backtesting:
            return self.kenbot_backtesting.backtesting.is_in_progress()
        else:
            return False

    def get_progress(self):
        if self.kenbot_backtesting.backtesting:
            return self.kenbot_backtesting.backtesting.get_progress()
        else:
            return 0
    ...
    async def _post_backtesting_end_callback(self):
        ...
        await self.kenbot_backtesting.stop_importers()
    ...
    def _init_default_config_values(self):
        self.risk = copy.deepcopy(self.kenbot_origin_config[common_constants.CONFIG_TRADING][
                                      common_constants.CONFIG_TRADER_RISK])
        self.starting_portfolio = copy.deepcopy(self.kenbot_origin_config[common_constants.CONFIG_SIMULATOR][
                                                    common_constants.CONFIG_STARTING_PORTFOLIO])
        self.fees_config = copy.deepcopy(self.kenbot_origin_config[common_constants.CONFIG_SIMULATOR][
                                             common_constants.CONFIG_SIMULATOR_FEES])
        if evaluator_constants.CONFIG_FORCED_TIME_FRAME in self.kenbot_origin_config:
            self.forced_time_frames = copy.deepcopy(self.kenbot_origin_config[
    ...
    def _get_exchanges_report(self, reference_market, trading_mode):
        ...
        starting_portfolios = {}
        end_portfolios = {}
        for exchange_id in self.kenbot_backtesting.exchange_manager_ids:
    ...
    def log_report(self):
        ...
        for exchange_id in self.kenbot_backtesting.exchange_manager_ids:
            ...
            self.logger.info(" ========= kenbot end state =========")
    ...
    def _log_global_report(self, exchange_manager):
        ...
        self.logger.info(f"Global market profitability (vs {reference_market}) : "
                         f"{market_average_profitability}% | kenbot : {profitability}%")

        self.logger.info(
            f"Simulation lasted "
            f"{round(backtesting_api.get_backtesting_duration(self.kenbot_backtesting.backtesting), 3)} sec")
