...
import kenbot
import kenbot.api.strategy_optimizer as strategy_optimizer_api
...
async def update_or_repair_tentacles_if_necessary(tentacles_setup_config, config):
    if not tentacles_manager_api.are_tentacles_up_to_date(tentacles_setup_config, constants.VERSION):
        logging.get_logger(COMMANDS_LOGGER_NAME).info("kenbot tentacles are not up to date. Updating tentacles...")
        if await install_or_update_tentacles(config):
            logging.get_logger(COMMANDS_LOGGER_NAME).info("kenbot tentacles are now up to date.")
    elif tentacles_manager_api.load_tentacles(verbose=True):
        logging.get_logger(COMMANDS_LOGGER_NAME).debug("kenbot tentacles are up to date.")
    else:
        logging.get_logger(COMMANDS_LOGGER_NAME).info("kenbot tentacles are damaged. Installing default tentacles ...")
...
async def install_all_tentacles(tentacles_url=None):
    ...
    async with aiohttp.ClientSession() as aiohttp_session:
        ...
        #                                                   aiohttp_session=aiohttp_session,
        #                                                   bot_install_dir=constants.kenbot_FOLDER)


def _signal_handler(_, __):
    # run Commands.BOT.stop_threads in thread because can't use the current asyncio loop
    stopping_thread = threading.Thread(target=kenbot.get_bot().task_manager.stop_tasks(),
...
def restart_bot():
    ...
    if get_bot_file().endswith(".py"):
        ...
        # restart from python kenbot package entrypoint
...
def update_bot(bot_api):
    import kenbot.updater.updater_factory as updater_factory
