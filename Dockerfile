...
COPY kenbot/config /kenbot/kenbot/config
...
RUN apt-get update \
    ...
    && ln -s /opt/venv/bin/kenbot kenbot # Make sure we use the virtualenv \
    && chmod +x docker-entrypoint.sh

VOLUME /kenbot/backtesting
VOLUME /kenbot/logs
VOLUME /kenbot/tentacles
VOLUME /kenbot/user
EXPOSE 5001
