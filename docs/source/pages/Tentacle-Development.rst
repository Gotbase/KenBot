...
A tentacle is part of a `tentacle package <Tentacle-Package-Development.html>`_ and defines a tool for kenbot
such as a way to analyse moving averages, listen to twitter for create grid-like orders.

kenbot uses tentacles to handle:
...
There is no limit to the things kenbot can handle: everything that can be coded can be used by kenbot through a
tentacle. It is possible to create a new tentacle to add a new tool to kenbot or to build on an existing one and
improve it.
...
.. code-block:: python
    ...
    class SuperSimpleStrategyEvaluator(SimpleStrategyEvaluator):
       ...
       # _trigger_evaluation is the methods called when kenbot is
