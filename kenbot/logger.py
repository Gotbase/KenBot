...
import kenbot_commons.logging as common_logging
import kenbot_commons.channels_name as channels_name
import kenbot_commons.pretty_printer as pretty_printer

import kenbot_evaluators.evaluators.channel as evaluator_channels

import kenbot_trading.exchange_channel as exchanges_channel
import kenbot_trading.enums as trading_enums
...
def init_logger():
    try:
        ...
        BOT_CHANNEL_LOGGER = common_logging.get_logger("kenbot Channel")
    except KeyError:
        print(
            "Impossible to start kenbot: the logging configuration can't be found in '"
            + constants.LOGGING_CONFIG_FILE
            + "' please make sure you are running kenbot from its root directory."
        )
        os._exit(-1)

    logger = logging.getLogger("kenbot Launcher")
    ...
    try:
        ...
        print(
            "Impossible to start kenbot: the logging file is locked, this is probably due to another running "
            "kenbot instance."
        )
        os._exit(-1)
...
async def init_exchange_chan_logger(exchange_id):
    if os.getenv(constants.ENV_ENABLE_DEBUG_LOGS, False):
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.MINI_TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            mini_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.RECENT_TRADES_CHANNEL.value,
                                         exchange_id).new_consumer(
            recent_trades_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.ORDER_BOOK_CHANNEL.value,
                                         exchange_id).new_consumer(
            order_book_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.ORDER_BOOK_TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            order_book_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.KLINE_CHANNEL.value,
                                         exchange_id).new_consumer(
            kline_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.OHLCV_CHANNEL.value,
                                         exchange_id).new_consumer(
            ohlcv_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.FUNDING_CHANNEL.value,
                                         exchange_id).new_consumer(
            funding_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.MARK_PRICE_CHANNEL.value,
                                         exchange_id).new_consumer(
            mark_price_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.BALANCE_CHANNEL.value,
                                     exchange_id).new_consumer(
        balance_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.BALANCE_PROFITABILITY_CHANNEL.value,
                                     exchange_id).new_consumer(
        balance_profitability_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.TRADES_CHANNEL.value,
                                     exchange_id).new_consumer(
        trades_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.LIQUIDATIONS_CHANNEL.value,
                                     exchange_id).new_consumer(
        liquidations_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.POSITIONS_CHANNEL.value,
                                     exchange_id).new_consumer(
        positions_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.kenbotTradingChannelsName.ORDERS_CHANNEL.value,
                                     exchange_id).new_consumer(
...
async def init_evaluator_chan_logger(matrix_id: str):
    await evaluator_channels.get_chan(channels_name.kenbotEvaluatorsChannelsName.MATRIX_CHANNEL.value,
                                      matrix_id).new_consumer(
        matrix_callback, priority_level=LOGGER_PRIORITY_LEVEL
    )
    await evaluator_channels.get_chan(channels_name.kenbotEvaluatorsChannelsName.EVALUATORS_CHANNEL.value,
                                      matrix_id).new_consumer(
        evaluators_callback, priority_level=LOGGER_PRIORITY_LEVEL
    )


async def init_kenbot_chan_logger(bot_id: str):
    await channel_instances.get_chan_at_id(constants.kenbot_CHANNEL, bot_id).new_consumer(
        kenbot_channel_callback,
        priority_level=LOGGER_PRIORITY_LEVEL,
        bot_id=bot_id,
        subject=[enums.kenbotChannelSubjects.NOTIFICATION.value, enums.kenbotChannelSubjects.ERROR.value]
    )
...
async def kenbot_channel_callback(
        bot_id: str,
        subject: str,
        action: str,
        data: dict
    ...
    BOT_CHANNEL_LOGGER.debug(
        f"kenbot_CHANNEL : SUBJECT = {subject} || ACTION = {action} || DATA = {data} "
    )
