...
import kenbot_commons.os_util as os_util
...
import kenbot.configuration_manager as configuration_manager
import kenbot.kenbot_backtesting_factory as kenbot_backtesting
import kenbot.constants as constants
import kenbot.disclaimer as disclaimer
import kenbot.logger as kenbot_logger


def update_config_with_args(starting_args, config: configuration.Configuration, logger):
    try:
        import kenbot_backtesting.constants as backtesting_constants
    except ImportError as e:
        logging.get_logger().error(
            "Can't start backtesting without the kenbot_backtesting package properly installed.")
        raise e
...
def _log_environment(logger):
    try:
        logger.debug(f"Running on {os_util.get_current_platform()} with {os_util.get_kenbot_type()}")
    except Exception as e:
...
def _validate_config(config, logger):
    try:
        ...
        if configuration_manager.migrate_from_previous_config(config):
            ...
            logger.error("kenbot can't repair your config.json file: invalid format: " + str(err))
            raise errors.ConfigError from err


def _repair_with_default_profile(config, logger):
    logger.error("kenbot can't start without a valid profile configuration. Selecting default profile ...")
...
def _load_or_create_tentacles(config, logger):
    ...
    if os.path.isfile(tentacles_manager_constants.USER_REFERENCE_TENTACLE_CONFIG_FILE_PATH):
        ...
        logger.info("kenbot tentacles can't be found. Installing default tentacles ...")
        commands.run_tentacles_install_or_update(config)
        config.load_profiles_if_possible_and_necessary()


def start_kenbot(args):
    logger = None
    try:
        if args.version:
        ...
        logger = kenbot_logger.init_logger()
        ...
        # create kenbot instance
        if args.backtesting:
            bot = kenbot_backtesting.kenbotBacktestingFactory(config,
                                                                run_on_common_part_only=not args.whole_data_range,
                                                                enable_join_timeout=args.enable_backtesting_timeout)
        else:
            bot = kenbot_class.kenbot(config, reset_trading_history=args.reset_trading_history)
        ...
        logger.error("kenbot can't start without a valid " + common_constants.CONFIG_FILE
                     + " configuration file." + "\nYou can use " +
                     constants.DEFAULT_CONFIG_FILE + " as an example to fix it.")
        os._exit(-1)

    except errors.NoProfileError:
        logger.error("kenbot can't start without a valid default profile configuration.")
        os._exit(-1)

    except ModuleNotFoundError as e:
        if 'tentacles' in str(e):
            logger.error("Impossible to start kenbot, tentacles are missing.\nTo install tentacles, "
        ...
        logger.error("kenbot can't start without a valid  configuration file.\n"
...
def kenbot_parser(parser):
    parser.add_argument('-v', '--version', help='Show kenbot current version.',
                        action='store_true')
    parser.add_argument('-s', '--simulate', help='Force kenbot to start with the trader simulator only.',
                        action='store_true')
    parser.add_argument('-u', '--update', help='Update kenbot to latest version.',
                        action='store_true')
    parser.add_argument('-rts', '--reset-trading-history', help='Force the traders to reset their history. They will '
                                                                ...
                                                                'fresh new portfolio.',
                        action='store_true')
    parser.add_argument('-b', '--backtesting', help='Start kenbot in backesting mode using the backtesting '
    ...
    parser.add_argument('-nw', '--no_web', help="Don't start kenbot web interface.",
                        action='store_true')
    parser.add_argument('-nt', '--no-telegram', help='Start kenbot without telegram interface, even if telegram '
                                                     'credentials are in config. With this parameter, your kenbot '
    ...
    parser.add_argument('--encrypter', help="Start the exchange api keys encrypter. This tool is useful to manually add"
                        ...
                        action='store_true')
    parser.add_argument('--identifier', help="kenbot community identifier.", type=str, nargs=1)
    parser.add_argument('-o', '--strategy_optimizer', help='Start kenbot strategy optimizer. This mode will make '
                                                           'kenbot play backtesting scenarii located in '
                        ...
                        nargs='+')
    parser.set_defaults(func=start_kenbot)

    # add sub commands
    subparsers = parser.add_subparsers(title="Other commands")

    # tentacles manager
    tentacles_parser = subparsers.add_parser("tentacles", help='Calls kenbot tentacles manager.\n'
...
def start_background_kenbot_with_args(version=False,
                                       update=False,
                                       encrypter=False,
    ...
    if in_subprocess:
        bot_process = multiprocessing.Process(target=start_kenbot, args=(args,))
        bot_process.start()
        return bot_process
    else:
        return start_kenbot(args)


def main(args=None):
    if not args:
        args = sys.argv[1:]
    parser = argparse.ArgumentParser(description='kenbot')
    kenbot_parser(parser)

    MIN_TENTACLE_MANAGER_VERSION = "1.0.10"

    # check compatible tentacle manager
    try:
        from kenbot_tentacles_manager import VERSION

        if LooseVersion(VERSION) < MIN_TENTACLE_MANAGER_VERSION:
            print("kenbot requires kenbot-Tentacles-Manager in a minimum version of " + MIN_TENTACLE_MANAGER_VERSION +
                  " you can install and update kenbot-Tentacles-Manager using the following command: "
                  "python3 -m pip install -U kenbot-Tentacles-Manager", file=sys.stderr)
            sys.exit(-1)
    except ImportError:
        print("kenbot requires kenbot-Tentacles-Manager, you can install it using "
              "python3 -m pip install -U kenbot-Tentacles-Manager", file=sys.stderr)
        sys.exit(-1)
