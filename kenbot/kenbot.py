...
import kenbot.community as community_manager
import kenbot.constants as constants
import kenbot.configuration_manager as configuration_manager
import kenbot.task_manager as task_manager
import kenbot.kenbot_channel_consumer as kenbot_channel_consumer
import kenbot.kenbot_api as kenbot_api
import kenbot.initializer as initializer
import kenbot.producers as producers
...
class kenbot:
    ...
    def __init__(self, config: configuration.Configuration, ignore_config=False, reset_trading_history=False):
        ...
        # Used to know when kenbot is ready to answer in APIs
        ...
        self.community_auth = community_manager.CommunityAuthentication(
            constants.kenbot_COMMUNITY_AUTH_URL,
            config=self.get_edited_config(constants.CONFIG_KEY, dict_only=False),
        )

        # kenbot_api to request the current instance
        self.kenbot_api = kenbot_api.kenbotAPI(self)

        # kenbot channel global consumer
        self.global_consumer = kenbot_channel_consumer.kenbotChannelGlobalConsumer(self)
    ...
    async def initialize(self):
        ...
        await logger.init_kenbot_chan_logger(self.bot_id)
        await self.create_producers()
        await self.start_producers()

    async def create_producers(self):
        self.exchange_producer = producers.ExchangeProducer(self.global_consumer.kenbot_channel, self,
                                                            None, self.ignore_config)
        self.evaluator_producer = producers.EvaluatorProducer(self.global_consumer.kenbot_channel, self)
        self.interface_producer = producers.InterfaceProducer(self.global_consumer.kenbot_channel, self)
        self.service_feed_producer = producers.ServiceFeedProducer(self.global_consumer.kenbot_channel, self)
    ...
    def _init_community(self):
        self.community_handler = community_manager.CommunityManager(self.kenbot_api)
